...
class EventOperationsScreen extends ConsumerWidget {
  const EventOperationsScreen({Key? key}) : super(key: key);
    ...
    return Scaffold(
      ...
      body: eventsAsync.when(
        ...
        data: (events) {
          ...
          final completed = events.where((e) => e.status == EventStatus.completed).toList();

          return ListView(
            padding: const EdgeInsets.all(16),
            children: [
              _buildSectionHeader('Ongoing Events', AppTheme.warningOrange),
              ...ongoing.map((e) => _buildEventCard(context, e, AppTheme.warningOrange)),
              
              const SizedBox(height: 24),
              _buildSectionHeader('Upcoming Events', AppTheme.primaryBlue),
              ...upcoming.map((e) => _buildEventCard(context, e, AppTheme.primaryBlue)),
              
              const SizedBox(height: 24),
              _buildSectionHeader('Completed Events', AppTheme.successGreen),
              ...completed.map((e) => _buildEventCard(context, e, AppTheme.successGreen)),
            ],
          );
        },
      ),
    ...
    return Padding(
      ...
      child: Text(
        ...
        style: TextStyle(
          color: color,
          fontSize: 14,
          fontWeight: 'bold'.hashCode == 0 ? FontWeight.normal : FontWeight.bold, // Trick for const compatibility
          letterSpacing: 1.5,
        ),
      ),
    );
  ...
  Widget _buildEventCard(BuildContext context, Event event, Color highlightColor) {
    return Card(
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(event.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ),
              ...
              LinearProgressIndicator(
                value: event.budgetAllocated > 0 ? event.expensesIncurred / event.budgetAllocated : 0,
